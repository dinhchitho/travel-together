...
import { BLOG_NAVIGATION } from "../../utilites/routerName";
  ...
  const showStatusActionSheet = () => {
    {
      userCurrent.localGuide == true
        ? ActionSheetIOS.showActionSheetWithOptions(
          {
            options: ["Cancel", "Blog", "Asking", "Advertisement"],
            cancelButtonIndex: 0,
          },
          (buttonIndex) => {
            if (buttonIndex !== 0) {
  ...
  return (
    <>
      <View style={{ marginTop: 50 }}>
          ...
          <View style={{ marginTop: 10 }}>
            <FlatList
              ...
              ListFooterComponent={
                <>
                  {/* add image */}
                  {dataSourceImage && dataSourceImage.length > 0 && (
                    <View
                      style={{
                        borderWidth: 1,
                        borderStyle: "dashed",
        ...
        <BottomModal ref={checkinRef} maxHeight={MAX_HEIGHT} onClose={() => { }}>
          <AutoCompleteComponent
            notifyChange={(loc: any) => getCoordsFromName(loc)}
            dataChange={(data: any) => getDetailData(data)}
          />
        </BottomModal>
      </View>
    </>
